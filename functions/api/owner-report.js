...
export async function onRequestGet({ request, env }) {
  ...
  const [events, channels, experiments, outreach, sales, recent] = await Promise.all([
    ...
    env.DB.prepare(`SELECT experiment,event_name,COUNT(*) count,
      COUNT(DISTINCT COALESCE(user_id,visitor_id)) people
      FROM funnel_events WHERE created_at >= datetime('now','-30 days') AND channel='teacher-outreach'
      GROUP BY experiment,event_name ORDER BY experiment,event_name`).all(),
  ...
  const resourceDownloads = funnel.resource_download && funnel.resource_download.people || 0;
  const resourceShares = funnel.resource_share && funnel.resource_share.people || 0;
  ...
  const upgradeViews = funnel.upgrade_view && funnel.upgrade_view.people || 0;
  ...
  for (const row of outreach.results || []) {
    const key = row.experiment || 'unattributed';
    teacherOutreach[key] = teacherOutreach[key] || {};
    teacherOutreach[key][row.event_name] = { events: row.count, people: row.people };
  }
  return json({
    generated_at: new Date().toISOString(), period_days: 30, funnel,
    conversion: {
      visit_to_resource_click: visits ? resourceClicks / visits : null,
      visit_to_resource_download: visits ? resourceDownloads / visits : null,
      visit_to_resource_share: visits ? resourceShares / visits : null,
      resource_click_to_practice: resourceClicks ? practiceStarts / resourceClicks : null,
      resource_download_to_practice: resourceDownloads ? practiceStarts / resourceDownloads : null,
      resource_share_to_practice: resourceShares ? practiceStarts / resourceShares : null,
      ...
      practice_to_upgrade_view: practiceStarts ? upgradeViews / practiceStarts : null,
      upgrade_view_to_checkout: upgradeViews ? checkouts / upgradeViews : null,
    ...
    teacher_outreach: teacherOutreach,
